...
import { DevFlux } from "@devflux/ai";

function App() {
  const [code, setCode] = useState("");

  const handleAICompletion = async () => {
    const suggestion = await DevFlux.complete(code);
    setCode(suggestion);
  };
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await DevFlux.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await DevFlux.search(searchQuery);
    return results;
  };
