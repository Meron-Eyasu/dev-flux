...
export default function Pricing(){
    return <section id="pricing" className="relative px-10 sm:px-6 lg:px-8 py-16 sm:py-20">
        <div className="max-w-7xl mx-auto">
            ...
            {/** PRICING GRIDS */}
            <div className="grid gird-cols-1 md:gird-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-6">
                {plans.map((plan, key) => (
                    <div key={key} className={`relative bg-slate-900/50 backdrop-blur-sm border rounded-xl
                        sm:rounded-2xl p-6 sm:p-8 transition-all duration-300 overflow-visible flex group flex-col h-full
                        ${plan.mostPopular? "border-blue-500 shadow-2xl shadow-blue-500/20 lg:scale-105": 
                        "border-slate-800 hover:border-slate-700"}`}>

                    <div className="absolute inset-0 bg-linear-to-br from-white/10 via-transparent 
                        to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 
                        transform -translate-x-full -translate-y-full group-hover:translate-x-0 
                        group-hover:translate-y-0 pointer-events-none rounded-lg" />        
                    {plan.mostPopular && (
                        <div className="absolute -top-2 sm:-top-3 left-1/2 transform -translate-x-1/2 z-10">
                            <div className="flex items-center space-x-1 px-3 sm:px-4 py-1 sm:py-1.5 
                                bg-linear-to-b from-blue-500 to-cyan-500 rounded-full text-xs sm:text-sm 
                        ...
                        <div className="text-center mb-6 sm:mb-8">
                            <h3 className="text-xl sm:text-2xl mb-2 font-bold">{plan.name}</h3>
                        ...
                        <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-row">
                            {plan.features.map((feature, featureKey) => (
                                <li key={featureKey} className="flex items-start space-x-2 sm:space-x-3 ">
                                    <div className="shrink-0 w-4 h-4 sm:w-5 sm:h-4 bg-blue-500/20 
                                        rounded-full flex items-center justify-center mt-0.5">
                                        <Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-blue-400"/>
                                    </div>
